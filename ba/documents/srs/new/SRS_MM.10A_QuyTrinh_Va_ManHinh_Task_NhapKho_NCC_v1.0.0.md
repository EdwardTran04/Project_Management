...
| **2a** | `NEW` | **Giám đốc kho Duyệt lịch giao việc** (Bước 3) | `UNASSIGNED` | Giám đốc kho | *(Áp dụng riêng cho Task 1)* Ngay khi GĐ kho duyệt lịch ➔ Hệ thống chuyển Task 1 sang `UNASSIGNED`, tính toán Deadline SLA và đẩy lên Pool việc chung để NV kho nhận việc hoặc Auto-Dispatch. |
...
| **3b** | `UNASSIGNED` | Hết 5 phút đếm ngược mà chưa có NV nào bấm nhận việc | `IN_PROGRESS` | Hệ thống (Auto-Assign Service) | *(Cơ chế Auto-Dispatching)* Hệ thống chạy thuật toán tự động giao việc tối ưu (ưu tiên FIFO rảnh trước, xét điều kiện xe nâng, hỗ trợ giao song song 2-3 người), phát thông báo high-priority. |
...
#### 2.2.5. Cơ chế Cấu hình & Công thức tính SLA theo Khối lượng Lệnh

##### A. Bảng Cấu hình SLA theo Dải Khối lượng (Weight-based SLA Matrix):
- **Căn cứ xác định khối lượng:** Hệ thống tính toán SLA căn cứ vào **Tổng khối lượng của toàn bộ Lệnh nhập kho** (`total_weight_kg = SUM(item.gross_weight_kg)` của Order từ bản tin SAP `T-API1`).
- **Phân loại Task và Cơ chế tính:**
  - **Nhóm Task tác nghiệp vật lý** (*Dỡ hàng `T-Unl`, Kiểm hàng `T-Ho`, Đưa bãi chờ nhập `T-Mv1`, Đưa bãi đóng gói `T-Mv2`, Đóng gói `T-Pac`, Cất hàng lưu trữ `T-Mv3`*): Thời lượng định mức SLA được tính linh hoạt theo **Dải khối lượng cấu hình**.
  - **Nhóm Task thủ tục & chứng từ** (*Ký BBBG điện tử, Trình ký V-Office `T-Sig`*): Thời lượng định mức SLA được thiết lập **Cố định (Fixed Duration)** độc lập với khối lượng hàng hóa.

- **Bảng ma trận cấu hình mẫu cho Task vật lý (Ví dụ: Task Dỡ hàng `[T-Unl]`):**

| Mã Dải Khối Lượng | Khoảng Khối Lượng Toàn Lệnh ($W$) | Thời lượng SLA Định Mức ($T_{\text{norm}}$) | Quy cách xử lý & Nhân sự mặc định |
|---|---|---|---|
| `W-RANGE-01` | $1 \text{ kg} \le W < 500 \text{ kg}$ | **30 phút** (0.5 giờ) | 1 Nhân viên bốc dỡ thủ công / xe đẩy |
| `W-RANGE-02` | $500 \text{ kg} \le W < 1,000 \text{ kg}$ | **60 phút** (1.0 giờ) | 1 Nhân viên kho |
| `W-RANGE-03` | $1,000 \text{ kg} \le W < 3,000 \text{ kg}$ | **120 phút** (2.0 giờ) | 1 Nhân viên + Xe nâng chuyên dụng |
| `W-RANGE-04` | $3,000 \text{ kg} \le W < 5,000 \text{ kg}$ | **240 phút** (4.0 giờ) | 1 Nhân viên + Xe nâng chuyên dụng |
| `W-RANGE-05` | $W \ge 5,000 \text{ kg}$ (Hàng lô lớn) | **Cấu hình mở rộng** ($T_{\text{norm}} = 240\text{ phút} + 60\text{ phút} / \text{mỗi 2 tấn}$) | Kích hoạt cơ chế giao việc song song |

##### B. Quy định Khung giờ làm việc & Lịch kho chuẩn (Working Calendar):
- **Khung giờ hành chính chuẩn:**
  - Ca Sáng: **`08:00 - 12:00`** (4.0 giờ).
  - Ca Chiều: **`13:30 - 17:30`** (4.0 giờ).
  - Tổng thời gian làm việc hiệu dụng: **8.0 giờ/ngày**.
- **Ngày làm việc trong tuần:** Từ **Thứ Hai đến Thứ Bảy** (Chủ Nhật và các ngày nghỉ lễ theo lịch Tập đoàn là thời gian không làm việc).
- **Buffer nghỉ ngơi giữa các Task:** Mặc định cộng thêm **`5 phút`** đệm nghỉ sau mỗi Task hoàn thành để chuyển giao công việc.

##### C. Công thức tính Deadline của Task và Lệnh:
1. **Thời điểm kết thúc dự kiến của Task ($Deadline_{\text{Task}}$):**
   $$Deadline_{\text{Task}} = \text{Thời điểm bắt đầu Task} + SLA_{\text{Task}} + \text{Thời gian ngoài giờ hành chính / Nghỉ trưa / Nghỉ CN (nếu có)}$$
2. **Thời điểm kết thúc dự kiến của toàn bộ Lệnh ($Deadline_{\text{Order}}$):**
   $$Deadline_{\text{Order}} = \text{Thời điểm bắt đầu Lệnh} + \sum_{i=1}^{N} SLA_{\text{Task}_i} + \sum_{i=1}^{N-1} (\text{Buffer 5 phút}) + \text{Thời gian ngoài giờ làm việc}$$
3. **Quy tắc xác định Trạng thái SLA:**
   - **`TRONG HẠN` (`ON_TIME`):** Khi $\text{Thời gian hiện tại} \le Deadline$.
   - **`QUÁ HẠN` (`OVERDUE`):** Khi $\text{Thời gian hiện tại} > Deadline$.
   *(Áp dụng đồng thời cho cả cấp độ từng Task và cấp độ toàn bộ Lệnh nhập kho).*

---

#### 2.2.6. Quy tắc Nghiệp vụ Gia hạn SLA/KPI (`[T-Extend]`)

Hệ thống cung cấp cơ chế cho phép Người phụ trách kho linh hoạt điều chỉnh thời hạn xử lý khi gặp sự cố khách quan tại hiện trường bãi kho (sự cố thiết bị nâng hạ, thời tiết, chờ kiểm đếm bổ sung):

1. **Giới hạn số lần:** Cho phép gia hạn **tối đa 01 lần / Task**. Sau khi đã gia hạn 1 lần, nút/chức năng gia hạn trên Task đó sẽ bị vô hiệu hóa (`disabled`).
2. **Thời điểm xin gia hạn:** Bắt buộc phải thực hiện **TRƯỚC KHI Task bị quá hạn** ($\text{Thời gian hiện tại} \le Deadline_{\text{Task}}$). Khi Task đã chuyển sang trạng thái `QUÁ HẠN`, hệ thống chặn không cho phép gửi yêu cầu gia hạn.
3. **Thời lượng gia hạn:** Người dùng tùy biến nhập số phút/giờ gia hạn theo nhu cầu thực tế, hệ thống không đặt giới hạn trần thời gian tối đa.
4. **Cơ chế Phê duyệt:** **Không cần phê duyệt** (Auto-Approved). Ngay khi người dùng xác nhận lý do và thời gian xin gia hạn, hệ thống tự động ghi nhận ngay lập tức.
5. **Cơ chế Lan truyền Thời hạn (Deadline Propagation):**
   ...
   - Lưu vết lịch sử: Ghi nhận mã nhân viên xin gia hạn, thời điểm, số phút gia hạn và lý do chi tiết vào bảng `task_sla_extension_log`.

---

#### 2.2.7. Cơ chế Giao việc Tự động & Giao việc Song song (Auto-Dispatching & Parallel Task Assignment)

Nhằm tối ưu hóa năng suất vận hành kho và đảm bảo hoàn thành Lệnh nhập/xuất trong ngày làm việc hành chính với thời gian trễ hạn ít nhất có thể (**Minimize Tardiness**):

##### A. Điều kiện Kích hoạt Giao việc Song song (Parallel Assignment Trigger):
Hệ thống tự động phân bổ nhiều nhân sự ($N \in \{1, 2, 3\}$) thực hiện cùng 1 Task khi thỏa mãn ít nhất một trong hai điều kiện sau:
1. **Điều kiện 1 (Thời gian hành chính còn lại không đủ):** Thời gian làm việc còn lại trong ca/ngày nhỏ hơn thời lượng $SLA_{\text{Task}}$ của 1 người.
2. **Điều kiện 2 (Ngưỡng thời lượng SLA của Task):**
   - **$SLA \le 4\text{ giờ}$:** Giao **1 người** phụ trách.
   - **$4\text{ giờ} < SLA \le 8\text{ giờ}$:** Giao **2 người** làm việc song song ➔ Thời lượng SLA sau điều chỉnh:
     $$SLA_{\text{mới}} = SLA_{\text{gốc}} \times 70\%$$
   - **$SLA > 8\text{ giờ}$:** Giao **3 người** làm việc song song ➔ Thời lượng SLA sau điều chỉnh:
     $$SLA_{\text{mới}} = SLA_{\text{gốc}} \times 45\%$$
   - **Trần số lượng:** Hệ thống cấu hình giới hạn tối đa **không vượt quá 3 người / 1 Task**.

##### B. Hình thức Hiển thị & Cơ chế Tương tác Song song:
- Trên hệ thống dữ liệu và giao diện Mobile/Web, công việc vẫn là **01 Task duy nhất** mang 1 Mã Task (`task_id`), nhưng trường danh sách người phụ trách lưu đồng thời mảng các ID nhân sự (`assignee_ids = ['NV1', 'NV2']`).
- Task hiển thị đồng thời trên màn hình làm việc của tất cả các nhân viên được gán.
- Bất kỳ nhân viên nào trong nhóm cũng có thể cập nhật tiến độ (quét barcode, nhập số lượng) và bấm **[Hoàn thành Task]** khi cả nhóm kết thúc công việc thực tế.

##### C. Điều kiện Thiết bị Chuyên dụng (Xe nâng / Forklift):
- Khi Lệnh có **Tổng khối lượng $> 1,000\text{ kg}$ (1 tấn)** HOẶC có gắn cờ **Hàng Quá khổ / Quá tải (`is_oversized_overweight = TRUE`)**:
  - Hệ thống chỉ lọc và phân công Task cho nhân viên có chứng chỉ/kỹ năng lái xe nâng hợp lệ trong hồ sơ (`has_forklift_license = TRUE`).

##### D. Ma trận Ưu tiên Giao việc Tự động (Priority Matrix):
Khi một nhân viên thỏa mãn điều kiện thực hiện nhiều Task khả dụng trong Pool việc chung:
1. **Ưu tiên 1 (Loại nghiệp vụ):** Ưu tiên **Task Xuất kho** trước **Task Nhập kho** (`OUTBOUND` > `INBOUND`).
2. **Ưu tiên 2 (Mức độ khẩn cấp):** Ưu tiên Task có **Thời gian Deadline gần nhất** (Thuật toán *Earliest Deadline First - EDF*).
3. **Ưu tiên 3 (Tối ưu thời gian trễ):** Lựa chọn phương án gán việc giúp giảm thiểu tối đa số phút vượt quá deadline (Minimize Total Tardiness).
4. **Ưu tiên 4 (Cân bằng nhân sự):** Ưu tiên chọn nhân viên có **thời gian chờ rảnh rỗi lâu nhất** (Thuật toán *FIFO Idle Workers*).
...
##### ① Thông tin chung Modal Gia hạn KPI (`mdl_extend_sla`)
- **Tác nhân:** Thủ kho (`ROLE_WAREHOUSE_MASTER`), Giám đốc kho (`ROLE_WAREHOUSE_DIRECTOR`), Nhân viên phụ trách Task (`ROLE_WAREHOUSE_WORKER`).
- **Mục đích:** Cho phép người dùng gia hạn thời gian hoàn thành của Task khi phát sinh sự cố khách quan tại hiện trường bãi kho.
- **Tiền điều kiện:** 
  1. Task chưa hoàn thành (`task_status IN ('UNASSIGNED', 'IN_PROGRESS')`).
  2. Task **chưa từng gia hạn** lần nào (`extension_count = 0`).
  3. Thời điểm xin gia hạn phải **trước khi Task bị quá hạn** (`NOW() <= task_deadline`).
- **Hậu điều kiện:** Task được gia hạn thời gian hoàn thành, `extension_count = 1`, Deadline của Task và Lệnh được tự động tịnh tiến thêm số phút gia hạn. Không cần qua phê duyệt (Auto-Approved).

##### ② Bảng Thành phần Giao diện Modal Gia hạn (`mdl_extend_sla`)

| STT | Tên Control | Kiểu Control | Input/Output | Ràng buộc dữ liệu & Validation | Mô tả chi tiết |
|---|---|---|---|---|---|
| 1 | `lbl_current_deadline` | Label / Text | Output | `DD/MM/YYYY HH:mm` | Hiển thị Deadline hiện tại của Task (VD: `08/09/2026 10:30`). |
| 2 | `num_extension_minutes` | Number Stepper / Input | Input | Bắt buộc, Integer $> 0$, Min: `5 phút`, Step: `5` | Số phút xin gia hạn (VD: `30`, `60`, `120`). Không giới hạn trần tối đa. |
| 3 | `cbo_extension_reason` | Dropdown Select | Input | Bắt buộc chọn | Danh mục lý do chuẩn: `SỰ CỐ XE NÂNG / THIẾT BỊ`, `THỜI TIẾT BÃI MƯA GIÓ`, `CHỜ PHÂN LOẠI / KIỂM ĐẾM BỔ SUNG`, `SỰ CỐ ĐƯỜNG TRUYỀN / MÁY QUÉT`, `LÝ DO KHÁC`. |
| 4 | `txt_manual_reason` | Text Area | Input | Tối đa 500 ký tự | Ghi chú diễn giải chi tiết nguyên nhân xin gia hạn. Bắt buộc nếu chọn lý do `LÝ DO KHÁC`. |
| 5 | `lbl_new_deadline_preview` | Dynamic Text | Output | `DD/MM/YYYY HH:mm` | Tự động tính và hiển thị Deadline mới dự kiến (tính cả giờ nghỉ hành chính nếu tràn giờ). |
| 6 | `btn_confirm_extend` | Primary Button (Đỏ) | Action | Active | Nút **[Xác nhận gia hạn]**. Gửi API `POST /api/v1/inbound/tasks/{id}/extend-sla`. |
| 7 | `btn_cancel_extend` | Outline Button | Action | Active | Nút **[Hủy bỏ]**. Đóng Modal. |

##### ③ Luồng Xử lý Backend & Giao tiếp API
- **Endpoint API:** `POST /api/v1/inbound/tasks/{id}/extend-sla`
- **Request Payload:**
  ```json
  {
    "extensionMinutes": 45,
    "reasonCode": "FORKLIFT_BREAKDOWN",
    "manualReason": "Xe nâng bãi 02 bị thủng lốp, cần 45 phút thay lốp dự phòng",
    "requestedBy": "NV111600"
  }
  ```
- **Business Logic & Validation:**
  ```sql
  -- 1. Kiểm tra điều kiện Task hợp lệ
  SELECT task_id, order_id, task_deadline, extension_count, task_status 
  FROM warehouse_task 
  WHERE task_id = :taskId;
  
  -- Nếu extension_count >= 1 -> Trả lỗi 400: "Task này đã được gia hạn 01 lần, không thể gia hạn thêm."
  -- Nếu NOW() > task_deadline -> Trả lỗi 400: "Task đã quá hạn SLA, không thể gửi yêu cầu gia hạn."
  -- Nếu task_status NOT IN ('UNASSIGNED', 'IN_PROGRESS') -> Trả lỗi 400: "Trạng thái Task không hợp lệ."

  -- 2. Cập nhật Deadline Task và số lần gia hạn
  UPDATE warehouse_task
  SET task_deadline = task_deadline + INTERVAL ':extensionMinutes minutes',
      extension_count = extension_count + 1,
      sla_status = 'ON_TIME',
      updated_at = NOW()
  WHERE task_id = :taskId;

  -- 3. Tịnh tiến Deadline toàn bộ Lệnh nhập kho tương ứng
  UPDATE warehouse_order
  SET order_deadline = order_deadline + INTERVAL ':extensionMinutes minutes',
      updated_at = NOW()
  WHERE order_id = :orderId;

  -- 4. Ghi log lịch sử gia hạn
  INSERT INTO task_sla_extension_log (
      task_id, order_id, original_deadline, extended_minutes, 
      new_deadline, reason_code, manual_reason, created_by, created_at
  ) VALUES (
      :taskId, :orderId, :originalDeadline, :extensionMinutes, 
      :newDeadline, :reasonCode, :manualReason, :requestedBy, NOW()
  );
  ```
- **Response Payload (HTTP 200 OK):**
  ...
  {
    "status": 200,
    "message": "Gia hạn SLA Task thành công",
    "data": {
      "taskId": "TSK-0012-01",
      "newDeadline": "2026-09-08T11:15:00Z",
      "extensionCount": 1,
      "orderNewDeadline": "2026-09-08T17:45:00Z",
      "slaStatus": "ON_TIME"
    }
  }
  ```
...
- **Mô tả chi tiết bố cục & thành phần màn hình:**
  ...
  4. **Khung Card 2 - Danh sách Chuỗi Người Phê duyệt V-Office (Approvers Chain Card):**
     - Hiển thị luồng phê duyệt 3 bước dạng Timeline:
       - *Bước 1 (Người khởi tạo):* `Phạm Trần Hùng - Thủ kho` (Ký nháy / Khởi tạo).
       - *Bước 2 (Kiểm soát):* `Nguyễn Văn Quản - Kế toán kho` (Ký kiểm soát hồ sơ).
       - *Bước 3 (Phê duyệt chính thức):* `Lê Văn Trưởng - Giám đốc kho / Trưởng đơn vị` (Ký số CA).
  5. **Khung Card 3 - Danh mục Tài liệu & Chứng từ Đính kèm (Attached Documents Card):**
     - Bảng danh mục tài liệu tự động gắn từ hệ thống:
       - File 1: `Phiếu nhập kho Mvt 101` (PDF kết xuất tự động từ SAP).
       - File 2: `Biên bản bàn giao BBBG Điện tử` (`BBBG-2026-0088.pdf` có đủ chữ ký 2 bên).
       - File 3: `Kết quả kiểm tra chất lượng KCS` (PDF từ phân hệ QM).
       - Nút `+ Thêm tài liệu đính kèm`: Cho phép tải thêm hình ảnh hiện trường hoặc hợp đồng.

#### ③ Bảng Ma Trận Control Chi Tiết (Control Matrix)

| STT | Tên Control | Kiểu Control | Input/Output | Ràng buộc dữ liệu & Validation | Mô Tả Chi Tiết |
|---|---|---|---|---|---|
| 1 | `txt_doc_title` | Textbox | Input | Bắt buộc, Max 255 ký tự | Tên hồ sơ trình ký V-Office. |
| 2 | `cbo_voffice_flow` | Dropdown Select | Input | Bắt buộc chọn | Chọn luồng trình ký mẫu tương ứng với loại kho và đơn vị. |
| 3 | `txt_summary` | Textarea | Input | Tối đa 1000 ký tự | Trích yếu nội dung trình ký. |
| 4 | `chk_urgent` | Checkbox | Input | Mặc định Unchecked | Tích chọn để gắn cờ Hỏa tốc trên V-Office. |
| 5 | `btn_preview_doc` | Outline Button | Action | Active | Mở modal xem trước toàn bộ file chứng từ PDF đã merge. |
| 6 | `btn_submit_voffice` | Primary Button (Đỏ) | Action | Active | Nút **[Gửi trình ký V-Office]**. Phát bản tin API `V-API1`. |

#### ④ Luồng xử lý nghiệp vụ
1. **Khởi tạo & Trình ký:** Thủ kho vào màn hình `SCR-WH-VOFFICE-SIG-01`, kiểm tra tài liệu tự động sinh, bấm **[Gửi trình ký V-Office]** ➔ Hệ thống phát bản tin `V-API1` tạo hồ sơ trên V-Office và chuyển trạng thái V-Office sang `Đang trình ký`.
2. **Tiếp nhận kết quả ký duyệt:** Khi lãnh đạo hoàn tất ký số CA trên V-Office ➔ V-Office gửi webhook callback `V-API2` về AI-WS. AI-WS cập nhật trạng thái `Đã ký duyệt` và phát bản tin `V-API3` sang SAP để chốt chứng từ kế toán.
3. **Trường hợp bị từ chối:** Nếu hồ sơ bị từ chối trên V-Office ➔ Webhook `V-API2` gửi lý do từ chối về AI-WS, hệ thống thông báo cho Thủ kho điều chỉnh hồ sơ và trình ký lại.

---

## PHẦN 4. THIẾT KẾ DÙNG CHUNG VÀ TÁI SỬ DỤNG

### 4.1. Bộ Quy tắc Dùng chung cho Quản lý SLA/KPI & Phân bổ Công việc

1. **Công thức tính SLA:** Toàn bộ các phân hệ Nhập kho, Xuất kho, Điều chuyển, Kiểm kê đều tái sử dụng chung bộ công thức tính SLA dựa trên:
   - Tổng khối lượng Lệnh (`total_weight_kg`).
   - Khung giờ làm việc hành chính (08:00 - 12:00, 13:30 - 17:30 từ Thứ 2 đến Thứ 7).
   - Buffer nghỉ ngơi 5 phút giữa các Task liên tiếp.
2. **Quy chuẩn Gia hạn KPI:** Tái sử dụng thống nhất rule:
   - Tối đa 01 lần / Task.
   - Bắt buộc gửi yêu cầu trước khi quá hạn (`NOW() <= deadline`).
   - Không giới hạn trần thời gian, tự động ghi nhận (Auto-Approved) và tịnh tiến Deadline Lệnh.
3. **Quy chuẩn Giao việc Song song & Tự động (Auto-Dispatching):**
   - $SLA \le 4\text{h}$: 1 người.
   - $4\text{h} < SLA \le 8\text{h}$: 2 người ($SLA \times 70\%$).
   - $SLA > 8\text{h}$: 3 người ($SLA \times 45\%$).
   - Điều kiện hàng $> 1\text{ tấn}$ / Quá khổ quá tải: Lọc nhân viên có kỹ năng Xe nâng (`has_forklift_license = TRUE`).
   - Ma trận ưu tiên: `OUTBOUND` > `INBOUND` $\rightarrow$ `Earliest Deadline First` $\rightarrow$ `Minimize Tardiness` $\rightarrow$ `FIFO Idle Workers`.
...
1. **Toàn vẹn Dữ liệu (Data Integrity):**
   - Mọi thay đổi trạng thái Order và Task đều được bọc trong Transaction DB với Lock cấp bản ghi (`SELECT ... FOR UPDATE`).
   - Tất cả các giao dịch đổi trạng thái, phân công, gia hạn KPI, và kết quả KCS đều được ghi log đầy đủ vào bảng `audit_logs` và `task_sla_extension_log`.
2. **Bảo mật & Phân quyền (Security & RBAC):**
   - Áp dụng kiểm tra quyền nghiêm ngặt theo Token JWT trên mọi Endpoint API.
   - Nhân viên chỉ được xem và cập nhật Task thuộc kho vật lý và ca làm việc được phân công.

---

## PHẦN 6. PHỤ LỤC

1. **Bản đồ API Tích hợp Đồng bộ:** [SRS_MM.10A_ChucNang_DongBo_ThongTin_NCC_v1.0.md](file:///c:/Users/Admin/Desktop/ai-agent-wms/ba/documents/srs/new/SRS_MM.10A_ChucNang_DongBo_ThongTin_NCC_v1.0.md)
2. **Tổng quan Dự án & Kiến trúc Tổng thể:** [AIWS_Project_Overview_And_Architecture.md](file:///c:/Users/Admin/Desktop/ai-agent-wms/knowledge/AIWS_Project_Overview_And_Architecture.md)
3. **Từ điển Dữ liệu & Cấu trúc Bảng Database:** [01_Entity_Attribute_Dictionary.md](file:///c:/Users/Admin/Desktop/ai-agent-wms/dev/db/01_Entity_Attribute_Dictionary.md)
