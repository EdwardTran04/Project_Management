...
document.addEventListener('DOMContentLoaded', () => {
  ...
  ORDER_DATA = MOCK.inboundOrders.find(o => o.id === ORDER_ID);
  if (!ORDER_DATA) {
    alert('Không tìm thấy dữ liệu lệnh nhập: ' + ORDER_ID);
    return;
  }
  ...
  document.getElementById('dt-order-type').textContent = ORDER_DATA.categoryLabel || 'INB-NCC';
  document.getElementById('dt-order-ref').textContent = ORDER_DATA.refCode;
  
  const supplier = MOCK.getSupplierById(ORDER_DATA.supplier);
  const supplierText = supplier ? supplier.name : (ORDER_DATA.supplierName || '—');
  document.getElementById('dt-order-supplier').textContent = supplierText;
  
  document.getElementById('dt-order-wh').textContent = ORDER_DATA.warehouse;
  document.getElementById('dt-order-plan-date').textContent = ORDER_DATA.inboundDate || '2026-05-18 09:00';
  
  const linesCount = ORDER_DATA.goods ? ORDER_DATA.goods.length : 12;
  document.getElementById('dt-order-lines').textContent = linesCount;
  
  const qtySum = ORDER_DATA.goods ? ORDER_DATA.goods.reduce((s, g) => s + (g.actual || g.doc || 0), 0) : 480;
...
function submitDecision() {
  if (DECISION === 'accept') {
    ...
    if (!date || !time) {
      alert('Vui lòng nhập đầy đủ ngày và giờ dự kiến nhập!');
      return;
    }
    ...
    if (ORDER_DATA) {
      ...
      ORDER_DATA.inboundDate = `${date} ${time}`;
    }

    alert(`Đồng ý tiếp nhận lệnh ${ORDER_ID} thành công! Hệ thống đã lập lịch nhập dự kiến: ${date} ${time}.`);
  } else {
    const reason = document.getElementById('input-reason').value.trim();
    if (!reason) {
      alert('Bắt buộc phải nhập lý do từ chối lệnh nhập!');
      return;
    }
...
function openFullDetail() {
  if (window.parent && window.parent !== window) {
    window.parent.postMessage({ action: 'loadDetail', url: `PH2_Nhap/inbound_detail.html?id=${ORDER_ID}` }, '*');
  } else {
    window.location.href = `inbound_detail.html?id=${ORDER_ID}`;
  }
}

function goBack() {
  if (window.parent && window.parent !== window) {
    window.parent.postMessage({ action: 'navigate', view: 'inbound-receipt' }, '*');
  } else {
    window.location.href = 'PH2.html';
  }
}
